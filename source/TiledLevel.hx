...
class TiledLevel extends TiledMap
{
    ...
    public var collisionMap:Array<Int>;
	...
	public function new(tiledLevel:Dynamic, state:PlayState)
	{
		...
		for (layer in layers)
		{
			...
			if (layer.name == "collisionmap") {
                this.collisionMap = tileLayer.tileArray.map(function(v){
                    if (v > 0) {
                        return 1; 
                    } else {
                        return 0;
	...
	private function loadObject(state:PlayState, o:TiledObject, g:TiledObjectLayer, group:FlxGroup)
	{
		...
		switch (o.type.toLowerCase())
		{
			case "player_start":
                ...
                bindings.addKey(ControlType.UP, FlxKey.W);
                bindings.addKey(ControlType.DOWN, FlxKey.S);
            ...
            case "player2_start":
                var bindings = new KeyBindings();
                bindings.addKey(ControlType.UP, FlxKey.UP);
                bindings.addKey(ControlType.DOWN, FlxKey.DOWN);
